...
def get_area_info():
    ...
    try:
        redis_store.setex(
            "area_info", constants.AREA_INFO_REDIS_CACHE_EXPIRES, resp_json)
    except Exception as e:
        current_app.logger.error(e)
...
def save_house_image():
    ...
    image_url = constants.FDFS_URL_DOMAIN + file_name
...
def get_house_index():
    ...
    if ret:
        ...
        try:
            redis_store.setex(
                "home_page_data", constants.HOME_PAGE_DATA_REDIS_EXPIRES, json_houses)
        except Exception as e:
            current_app.logger.error(e)
...
# GET /api/v1.0/houses/<int:house_id>
...
def get_house_list():
    ...
    # 区域条件
    if area_id:
        # python中使用==时会调用对象的__eq__方法
        # sqlalchemy重写Column对象的__eq__方法返回查询表达式
    ...
    # 处理分页
    try:
        # filter只是形成查询条件，还没有进行查询操作，paginate时才进行查询操作
